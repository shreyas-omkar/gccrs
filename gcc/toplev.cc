...
get_random_seed(bool noinit) {
  if (!random_seed && !noinit) {
    ...
    if (fd >= 0) {
      if (read(fd, &random_seed, sizeof(random_seed)) != sizeof(random_seed))
        random_seed = 0;
      close(fd);
    }
    if (!random_seed)
...
void wrapup_global_declaration_1(tree decl) {
  ...
  if (HAS_DECL_ASSEMBLER_NAME_P(decl) && DECL_DEFER_OUTPUT(decl) != 0)
  ...
  if (VAR_P(decl) && TREE_STATIC(decl)) {
    ...
    if (!node && flag_ltrans)
      ...
      needed = false;
             ...
             (TREE_USED(decl) || TREE_USED(DECL_ASSEMBLER_NAME(decl))))
      /* needed */;
      ...
      needed = false;
             ...
             (optimize || !flag_keep_static_consts || DECL_ARTIFICIAL(decl)))
      needed = false;
...
static void compile_file(void) {
  ...
  if (flag_generate_lto && !flag_fat_lto_objects) {
    ...
    ASM_OUTPUT_ALIGNED_COMMON(asm_out_file, "__gnu_lto_slim", HOST_WIDE_INT_1U,
                              8);
#else
    ASM_OUTPUT_COMMON(asm_out_file, "__gnu_lto_slim", HOST_WIDE_INT_1U,
                      HOST_WIDE_INT_1U);
#endif
  }
...
void print_version(FILE *file, const char *indent, bool show_global_state) {
  ...
  static const char fmt3[] = N_(
      "%s%swarning: %s header version %s differs from library version %s.\n");
  static const char fmt4[] = N_("%s%sGGC heuristics: --param ggc-min-expand=%d "
                                "--param ggc-min-heapsize=%d\n");
#ifndef __VERSION__
#define __VERSION__ "[?]"
#endif
  fprintf(file, file == stderr ? _(fmt1) : fmt1, indent,
          *indent != 0 ? " " : "", lang_hooks.name, pkgversion_string,
          version_string, TARGET_NAME, indent, __VERSION__);
...
#define GCC_GMP_VERSION                                                        \
  GCC_GMP_VERSION_NUM(__GNU_MP_VERSION, __GNU_MP_VERSION_MINOR,                \
...
#define GCC_GMP_STRINGIFY_VERSION                                                        \
  GCC_GMP_STRINGIFY_VERSION2(__GNU_MP_VERSION)                                           \
  "." GCC_GMP_STRINGIFY_VERSION2(__GNU_MP_VERSION_MINOR) "." GCC_GMP_STRINGIFY_VERSION2( \
      __GNU_MP_VERSION_PATCHLEVEL)
#endif
  fprintf(file, file == stderr ? _(fmt2) : fmt2, GCC_GMP_STRINGIFY_VERSION,
          MPFR_VERSION_STRING, MPC_VERSION_STRING,
#ifndef HAVE_isl
          "none"
#else
          isl_version()
  ...
  if (strcmp(GCC_GMP_STRINGIFY_VERSION, gmp_version))
    fprintf(file, file == stderr ? _(fmt3) : fmt3, indent,
            *indent != 0 ? " " : "", "GMP", GCC_GMP_STRINGIFY_VERSION,
            gmp_version);
  if (strcmp(MPFR_VERSION_STRING, mpfr_get_version()))
    fprintf(file, file == stderr ? _(fmt3) : fmt3, indent,
            *indent != 0 ? " " : "", "MPFR", MPFR_VERSION_STRING,
            mpfr_get_version());
  if (strcmp(MPC_VERSION_STRING, mpc_get_version()))
    fprintf(file, file == stderr ? _(fmt3) : fmt3, indent,
            *indent != 0 ? " " : "", "MPC", MPC_VERSION_STRING,
            mpc_get_version());

  if (show_global_state) {
    fprintf(file, file == stderr ? _(fmt4) : fmt4, indent,
            *indent != 0 ? " " : "", param_ggc_min_expand,
...
static void init_asm_output(const char *name) {
  if (name == NULL && asm_file_name == 0)
    ...
    if (!strcmp(asm_file_name, "-"))
      ...
      fatal_error(UNKNOWN_LOCATION, "input file %qs is the same as output file",
                  asm_file_name);
    if (asm_out_file == 0)
      fatal_error(UNKNOWN_LOCATION, "cannot open %qs for writing: %m",
                  asm_file_name);
  }
  ...
  if (!flag_syntax_only && !(global_dc->get_lang_mask() & CL_LTODump)) {
    ...
    if (flag_record_gcc_switches) {
      if (targetm.asm_out.record_gcc_switches) {
        const char *str = gen_producer_string(
...
static void output_stack_usage_1(FILE *cf) {
  ...
  const char *stack_usage_kind_str[] = {"static", "dynamic", "dynamic,bounded"};
  ...
  if (cf && flag_callgraph_info & CALLGRAPH_INFO_STACK_USAGE)
    fprintf(cf, "\\n" HOST_WIDE_INT_PRINT_DEC " bytes (%s)", stack_usage,
  ...
  if (warn_stack_usage >= 0 && warn_stack_usage < HOST_WIDE_INT_MAX) {
    ...
    if (stack_usage_kind == DYNAMIC)
      ...
      if (stack_usage_kind == DYNAMIC_BOUNDED)
        warning_at(loc, OPT_Wstack_usage_, "stack usage might be %wu bytes",
                   stack_usage);
      else
...
static void dump_final_node_vcg(FILE *f) {
  ...
  if (!node)
    return;
  node->call_for_symbol_thunks_and_aliases(dump_final_alias_vcg, f, true,
                                           false);
}
...
static void internal_error_reentered(diagnostics::context *, const char *,
                                     va_list *) {
...
static void internal_error_function(diagnostics::context *, const char *,
                                    va_list *) {
...
static void general_init(const char *argv0, bool init_signals,
  ...
  global_dc->set_path_format((enum diagnostic_path_format)global_options_init
  ...
  global_dc->m_show_column = global_options_init.x_flag_show_column;
  ...
  global_dc->set_option_id_manager(
      std::make_unique<compiler_diagnostic_option_id_manager>(
...
static void process_options() {
...
#ifndef HAVE_isl
  if (flag_graphite || flag_loop_nest_optimize || flag_graphite_identity ||
  ...
  if (flag_cf_protection != CF_NONE && !(flag_cf_protection & CF_SET)) {
  ...
  if (!lang_GNU_C() && ctf_debug_info_level > CTFINFO_LEVEL_NONE) {
  ...
  if (flag_dump_final_insns && !flag_syntax_only && !no_backend) {
    ...
    if (!final_output) {
      error_at(UNKNOWN_LOCATION, "could not open final insn dump file %qs: %m",
  ...
  if (!OPTION_SET_P(debug_variable_location_views)) {
    debug_variable_location_views =
        (flag_var_tracking && debug_info_level >= DINFO_LEVEL_NORMAL &&
         dwarf_debuginfo_p() && !dwarf_strict);
  ...
  if (debug_internal_reset_location_views == 2) {
    debug_internal_reset_location_views =
        (debug_variable_location_views && targetm.reset_location_view);
  ...
  if (flag_gen_aux_info) {
    ...
    if (aux_info_file == 0)
      fatal_error(UNKNOWN_LOCATION, "cannot open %s: %m", aux_info_file_name);
  }
  ...
  if ((flag_sanitize & SANITIZE_ADDRESS) && !FRAME_GROWS_DOWNWARD) {
    warning_at(UNKNOWN_LOCATION, 0,
  ...
  if (profile_flag || patch_area_size || !targetm.have_prologue() ||
  ...
  if (!OPTION_SET_P(warnings_are_errors)) {
    if (warn_coverage_mismatch && option_unspecified_p(OPT_Wcoverage_mismatch))
    ...
    if (warn_coverage_invalid_linenum &&
      ...
      diagnostic_classify_diagnostic(
          global_dc, OPT_Wcoverage_invalid_line_number,
          diagnostics::kind::error, UNKNOWN_LOCATION);
  }
  ...
  if (flag_checking >= 2)
    hash_table_sanitize_eq_limit = param_hash_table_verification_limit;
...
static int lang_dependent_init(const char *name) {
...
static bool standard_type_bitsize(int bitsize) {
  ...
  if (bitsize == 128)
    return false;
  if (bitsize == CHAR_TYPE_SIZE || bitsize == SHORT_TYPE_SIZE ||
      bitsize == INT_TYPE_SIZE || bitsize == LONG_TYPE_SIZE ||
...
int toplev::main(int argc, char **argv) {
  ...
  decode_cmdline_options_to_array_default_mask(
      argc, CONST_CAST2(const char **, char **, argv), &save_decoded_options,
      &save_decoded_options_count);
  ...
  decode_options(&global_options, &global_options_set, save_decoded_options,
                 save_decoded_options_count, UNKNOWN_LOCATION, global_dc,
                 targetm.target_option.override);
